...
def dumps_matrix(arr, gtsp_set, n_sets, name="route"):
    ...
    for i, row in enumerate(arr.tolist()):
        ...
        matrix_s += "\n"
    gtsp_set_s = ""
    for i, row in enumerate(gtsp_set):
        gtsp_set_s += " ".join([str(i) for i in row])
        gtsp_set_s += "\n"

    return template.format(**{'name': name,
                              'n_cities': n_cities,
                              'matrix_s': matrix_s,
                              'gtsp_set_s': gtsp_set_s,
                              'n_sets': n_sets})
...
def run(tsp_path, start=None, solver="concorde"):
    ...
    if solver.lower() == 'concorde':
        ...
        try:
            ...
            if "No such file or directory" in str(exc):
                raise TSPSolverNotFound(
                    "{0} is not found on your path or is not executable".format(GLKH))
        meta = []
        raw = []
        solution = None
